...
class Cate_Model extends API_Model {
    function __construct()
    {
        parent::__construct('tbl_category');
        $this->_select = array('id','title','data','pid','tid','sorting');
    }
    ...
    function buildTree(array $elements, $pid = 0,$parents=array(0)) {
        ...
        foreach ($elements as $element) {
            if ($element->pid == $pid) {
                $tmp=$parents;
                $tmp[]= +$element->id;
                $items = $this->buildTree($elements, $element->id,$tmp);
                if ($items) {
                    $element->items = $items;
                }
